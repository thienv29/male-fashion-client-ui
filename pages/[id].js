...
export default function ProductDetail() {
    ...
    const handleAddToCart = async () => {
        if (validateProductDetail()) {
            const productDetailCreated = listDetails.find(
                (detail) => detail.size._id == sizeSelected && detail.color._id == colorSelected
            );
            const data = {
            ...
            listDetails.forEach(async (detail) => {
                if (detail.color._id == colorSelected && detail.size._id == sizeSelected) {
                    if (data.quantity <= detail.stock) {
                        await CartService.create(data);

                        console.log(listDetails);
                        console.log(data);
                    } else {
                        alert('Quantity was out of stock');
                    }
                }
            });
        }
                                        ...
                                        <div className="d-flex mb-3">
                                            <div className="item-sku">
                                                ...
                                                >
                                                    {listDetails
                                                        ? listDetails.map((detail) =>
                                                              detail.color._id == colorSelected &&
                                                              detail.size._id == sizeSelected
                                                                  ? detail.code
                                                                  : ''
                                                          )
                                                        : ''}
                                            ...
                                            <div className="rating">
                                                <i
                                                    className={
                                                        1 <= star ? 'fa fa-star' : 'fa fa-star-o'
                                                    }
                                                />
                                                <i
                                                    className={
                                                        2 <= star ? 'fa fa-star' : 'fa fa-star-o'
                                                    }
                                                />
                                                <i
                                                    className={
                                                        3 <= star ? 'fa fa-star' : 'fa fa-star-o'
                                                    }
                                                />
                                                <i
                                                    className={
                                                        4 <= star ? 'fa fa-star' : 'fa fa-star-o'
                                                    }
                                                />
                                                <i
                                                    className={
                                                        5 <= star ? 'fa fa-star' : 'fa fa-star-o'
                                                    }
                                                />
                                                <span> - 5 Reviews</span>
                                            </div>
                                        ...
                                        <div className="product__details__text">
                                            <div className="d-flex justify-content-between">
                                                <h3 className="text-left">
                                                    ${salePrice == 0 ? exportPrice : salePrice}{' '}
                                                    <span>
                                                        ${salePrice == 0 ? '' : exportPrice}
                                                    </span>
                                                </h3>
                                                <div className=" text-left">
                                                    <a
                                                        className=" size-heart-icon"
                                                    ...
                                                    {sizes
                                                        ? sizes.map((detail) => (
                                                              <label
                                                                  ...
                                                                  htmlFor={detail.name}
                                                                  className={
                                                                      sizeSelected == detail._id
                                                                          ? 'active'
                                                                          : ''
                                                                  }
                                                                  onClick={() =>
                                                                      handleClickSize(detail._id)
                                                                  }
                                                              >
                                                                  {detail.name}
                                                                  <input
                                            ...
                                            <p className="text-left product__details__option__description">
                                                {description}
                                            </p>
